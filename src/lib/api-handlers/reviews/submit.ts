...
async function responseForProduct(productId: string, review: ProductReview) {
  const reviews = (await getFirebaseDb().collection("reviews").where("productId", "==", productId).where("status", "==", "published").get()).docs.map((doc) => ({ id: doc.id, ...doc.data() } as ProductReview));
  const stats = getReviewStats(reviews);
  return NextResponse.json({ ok: true, review, reviews, reviewCount: stats.reviewCount, rating: stats.averageRating });
}

/** POST /api/reviews/submit */
export async function handleReviewsSubmit(request: Request) {
  try {
