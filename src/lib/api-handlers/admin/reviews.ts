...
/** PATCH /api/admin/reviews — moderate a review (published/hidden) */
export async function handleAdminReviewPatch(request: Request) {
  try {
    await requireAdmin();
...
/** POST /api/admin/reviews — create a review (as the store team) */
export async function handleAdminReviewCreate(request: Request) {
  try {
    await requireAdmin();
    const {
      productId,
