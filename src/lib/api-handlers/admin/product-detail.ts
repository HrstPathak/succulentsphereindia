...
/** GET /api/admin/products/[id] — fetch a single product with its reviews */
export async function handleAdminProductDetail(id: string) {
  try {
    await requireAdmin();
...
/** PATCH /api/admin/products/[id] — update a product (supports multipart FormData image upload) */
export async function handleAdminProductUpdate(request: Request, id: string) {
  try {
    await requireAdmin();
