...
export async function handleAdminProductsList() {
  try {
    await requireAdmin();
...
/** POST /api/admin/products — create a product (supports multipart FormData image upload) */
export async function handleAdminProductCreate(request: Request) {
  try {
    await requireAdmin();
...
/** PATCH /api/admin/products — bulk patch selected products by ids */
export async function handleAdminProductsPatch(request: Request) {
  try {
    await requireAdmin();
