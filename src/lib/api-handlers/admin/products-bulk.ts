...
/**
 * POST /api/admin/products/bulk — batch update multiple products.
 * SECURITY FIX (intentional): the original handler had NO requireAdmin() check.
 * requireAdmin() has been added here so this endpoint now enforces admin
 * authorization exactly like every other /api/admin/* endpoint.
 */
export async function handleAdminProductsBulk(req: NextRequest) {
  try {
    await requireAdmin();
