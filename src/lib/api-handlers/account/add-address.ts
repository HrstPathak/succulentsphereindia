...
/** POST /api/account/add-address */
export async function handleAccountAddAddress(request: Request) {
  try {
    ...
    await createAddress(session.uid, address);
    return NextResponse.json({ ok: true, customer: await fetchCustomerByUid(session.uid) });
  } catch (error) {
    return NextResponse.json({ error: (error as Error).message || "Unable to add address." }, { status: 500 });
  }
}