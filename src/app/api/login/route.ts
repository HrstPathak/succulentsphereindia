...
export async function POST(request: Request) {
  try {
    ...
    ensureUserProfile({ uid: String(credential.localId), email: normalisedEmail, displayName: String(credential.displayName || "") }).catch((error) => {
      console.info(`[firebase login] profile sync skipped: ${String((error as Error)?.message || error)}`);
    });
    return response;
  } catch (error) {
