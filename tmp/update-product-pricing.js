...
async function updateProduct(target) {
  ...
  for (const doc of snapshot.docs) {
    ...
    const normalizedDoc = normalizeTitle(title);
    if (normalizedDoc === normalizedTarget) {
      matches.push(doc);
    }
  }

  if (!matches.length) {
    return { status: 'missing', title: target.title };
  }

  for (const doc of matches) {
    const data = doc.data();
    const available = target.availability.toLowerCase() === 'in stock';
    const update = {
      price: Number(target.salePrice),
      compareAtPrice: Number(target.regularPrice),
      available,
      inventoryQuantity: available ? 1 : 0,
      status: available ? 'active' : 'sold out',
  ...
  return { status: 'updated', title: target.title, matches: matches.length };
}

(async () => {
  const result = [];
  for (const target of updates) {
    result.push(await updateProduct(target));
  }
  const summary = {
    totalRequested: updates.length,
    updated: result.filter((entry) => entry.status === 'updated').length,
    missing: result.filter((entry) => entry.status === 'missing').length,
    details: result,
  };
